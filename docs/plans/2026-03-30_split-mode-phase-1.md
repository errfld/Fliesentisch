...
- `backend` now stores `gamemaster` vs `player` roles in SQLite bootstrap data, and the auth/token path exposes trusted `game_role` for authenticated sessions.
...
- The original phase-1 auth blocker has been addressed; the remaining split-mode work can build on the authenticated token/session plumbing already in place.
...
- The remaining blocker is clearly isolated to later transport/security hardening, not mixed into frontend state code.
